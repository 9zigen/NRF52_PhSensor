...
static void on_write(ble_cus_t * p_cus, ble_evt_t const * p_ble_evt)
{
  ...
  if (p_evt_write->handle == p_cus->status_handles.value_handle)
  {
    ...
    if(p_evt_write->data != NULL)
    {
      ...
      if (*p_evt_write->data == 1) {
        evt.evt_type = BLE_CUS_EVT_CALIBRATE_PH_LOW;
        NRF_LOG_INFO("CUS EVT: BLE_CUS_EVT_CALIBRATE_PH_LOW\r\n");
        do_ph_calibration(PH_LOW_CALIBRATION);

      } else if (*p_evt_write->data == 2) {
        evt.evt_type = BLE_CUS_EVT_CALIBRATE_PH_MID;
        NRF_LOG_INFO("CUS EVT: BLE_CUS_EVT_CALIBRATE_PH_MID\r\n");
        do_ph_calibration(PH_MID_CALIBRATION);
      ...
      )
  {
    ...
    if (p_cus->evt_handler != NULL)
    {
      ...
      if (ble_srv_is_notification_enabled(p_evt_write->data))
      {
        evt.evt_type = BLE_CUS_EVT_PH_NOTIFICATION_ENABLED;
      }
      else
      {
...
static uint32_t status_char_add(ble_cus_t * p_cus, const ble_cus_init_t * p_cus_init)
{
    ...
    initial_status_value = p_cus_init->initial_status_value;
    ...
    attr_char_value.p_value   = &initial_status_value;
...
static uint32_t temperature_char_add(ble_cus_t * p_cus, const ble_cus_init_t * p_cus_init)
{
  ...
  ble_uuid_t          ble_uuid;
  ble_gatts_attr_md_t attr_md;
  uint16_t            initial_temperature_value;
  ...
  initial_temperature_value = p_cus_init->initial_temperature_value;
  ...
  attr_char_value.p_value   = (uint8_t *)&initial_temperature_value;
 ...
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t ph_char_add(ble_cus_t *p_cus, const ble_cus_init_t *p_cus_init)
{
  uint32_t            err_code;
  ble_gatts_char_md_t char_md;
  ble_gatts_attr_md_t cccd_md;
  ...
  cccd_md.write_perm = p_cus_init->ph_char_attr_md.cccd_write_perm;
  cccd_md.vloc       = BLE_GATTS_VLOC_STACK;
  ...
  ble_uuid.type = p_cus->uuid_type;
  ble_uuid.uuid = PH_CHAR_UUID;

  memset(&attr_md, 0, sizeof(attr_md));

  attr_md.read_perm  = p_cus_init->ph_char_attr_md.read_perm;
  attr_md.write_perm = p_cus_init->ph_char_attr_md.write_perm;
  attr_md.vloc       = BLE_GATTS_VLOC_STACK;
  attr_md.rd_auth    = 0;
  attr_md.wr_auth    = 0;
  attr_md.vlen       = 0;

  initial_ph_value = p_cus_init->initial_ph_value;
  ...
  attr_char_value.init_offs = 0;
  attr_char_value.max_len   = 2;
  attr_char_value.p_value   = (uint8_t *)&initial_ph_value;
