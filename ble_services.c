...
#define DEVICE_NAME                     "PHSens"                                /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME               "NordicSemiconductor"                   /**< Manufacturer. Will be passed to Device Information Service. */
#define APP_ADV_INTERVAL                MSEC_TO_UNITS(180, UNIT_0_625_MS)       /**< The advertising interval (in units of 0.625 ms. This value corresponds to 187.5 ms). */
#define APP_ADV_SLOW_INTERVAL           MSEC_TO_UNITS(1280, UNIT_0_625_MS)      /**< The advertising interval (in units of 0.625 ms. This value corresponds to 187.5 ms). */

#define APP_ADV_DURATION                1800                                    /**< The advertising duration (180 seconds) in units of 10 milliseconds. */
#define APP_ADV_SLOW_DURATION           BLE_GAP_ADV_TIMEOUT_GENERAL_UNLIMITED   /**< The advertising duration (180 seconds) in units of 10 milliseconds. */
...
#define ADV_INTERVAL                    APP_TIMER_TICKS(10000)

NRF_BLE_GATT_DEF(m_gatt);                                                       /**< GATT module instance. */
NRF_BLE_QWR_DEF(m_qwr);                                                         /**< Context for the Queued Write module.*/
BLE_ADVERTISING_DEF(m_advertising);                                             /**< Advertising module instance. */
BLE_CUS_DEF(ble_cus);                                                           /**< Custom service instance. */
...
/* Function for handling the Battery Service events. */
static void on_bas_evt(ble_bas_t * p_bas, ble_bas_evt_t * p_evt)
{
  switch (p_evt->evt_type)
  {
    case BLE_BAS_EVT_NOTIFICATION_ENABLED:
      update_sensor_characteristic_start(BATTERY_SENSOR);
      break;

    case BLE_BAS_EVT_NOTIFICATION_DISABLED:
      update_sensor_characteristic_stop(BATTERY_SENSOR);
      break;
...
static void on_cus_evt(ble_cus_t * p_bas, ble_cus_evt_t * p_evt)
{
  NRF_LOG_INFO("CUS Event Handler, event type %d\r\n", p_evt->evt_type);

  switch(p_evt->evt_type)
  {
    case BLE_CUS_EVT_TEMPERATURE_NOTIFICATION_ENABLED:
      update_sensor_characteristic_start(TEMPERATURE_SENSOR);
      break;

    case BLE_CUS_EVT_PH_NOTIFICATION_ENABLED:
...
void services_init(void)
{
  ...
  BLE_GAP_CONN_SEC_MODE_SET_OPEN(&cus_init.status_char_attr_md.write_perm);
  cus_init.initial_status_value = 0;
  ...
  cus_init.initial_temperature_value = 25;

  /* PH */
  BLE_GAP_CONN_SEC_MODE_SET_OPEN(&cus_init.ph_char_attr_md.cccd_write_perm);
  BLE_GAP_CONN_SEC_MODE_SET_OPEN(&cus_init.ph_char_attr_md.read_perm);
...
void advertising_init(void)
{
  ...
  /* temp */
  data[1]                             = temperature.u8[0];
  data[2]                             = temperature.u8[1];
...
void adv_manuf_data_update()
{
  ...
  /* temp */
  manuf_data_array[1] = temperature.u8[0];
  manuf_data_array[2] = temperature.u8[1];
...
/* Function for update sensor data char. */
void update_sensors_service(sensor_t sensor_id)
{
  ...
  switch (sensor_id)
  {
    ...
    case TEMPERATURE_SENSOR:
      err_code = ble_cus_temperature_update(&ble_cus, temperature);
      APP_ERROR_CHECK(err_code);
      break;
